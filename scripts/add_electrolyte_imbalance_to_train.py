"""
Phase 4 — Add Electrolyte imbalance disease to scoring engine source data
==========================================================================

Usage (from project root):
    python scripts/add_electrolyte_imbalance_to_train.py
"""
...
DISEASE_NAME = "Electrolyte imbalance"
N_ROWS = 100

SYMPTOM_COUNTS: dict[str, int] = {
    "cramps":             90,   # hallmark · esp K/Ca low
    "fatigue":            80,
    "muscle_weakness":    75,
    "palpitations":       70,   # irregular K/Mg
    "dizziness":          60,
    "weakness_in_limbs":  55,
    "nausea":             55,
    "vomiting":           45,   # often cause
    "headache":           40,
}
...
def main() -> int:
    ...
    for sym, expected_count in SYMPTOM_COUNTS.items():
        ...
        mark = "✓" if ok else "✗"
        print(f"  {mark} {sym:20s}: {actual}/{len(target)} = {actual/len(target):.2f} (expected {expected_count/N_ROWS:.2f})")
        if not ok:
            all_ok = False
    if not all_ok:
        return 1
