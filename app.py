...
with col1:
    st.markdown(
        ...
        ทุกวันนี้เวลาไม่สบาย คนไทยมักเปิด Google ค้นหาอาการของตัวเองก่อน —
        แต่ผลที่ได้มักเป็นข้อมูลทางการแพทย์ที่อ่านยาก ไม่ตรงบริบท หรืออ่านแล้วยิ่งกังวล
        สุดท้ายก็ยังไม่รู้ว่าควรไปหาหมอแผนกไหน หรือเร่งด่วนแค่ไหน

        ### สิ่งที่แอปนี้ทำให้

        1. **แนะนำแผนก/ประเภทแพทย์** ที่ควรไปจากอาการที่ระบุ
        2. **ระดับความเร่งด่วน 1–5** อิง ED Triage MOPH (1 = ฉุกเฉินทันที, 5 = ไม่เร่งด่วน)
        3. **เปรียบเทียบ 2 โหมด** Non-AI (rule-based) vs AI (LLM + RAG)
