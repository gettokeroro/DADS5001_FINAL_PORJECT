...
class ScoringArtifacts:
    ...
    diseases: list[str]                    # 43 unique disease names (Phase 4: +Influenza, +Dehydration)
...
SELECT
  pd.disease,
  ROUND(pd.log_likelihood + LN(COALESCE(pn.prior_p, 1.0/43)), 3) AS log_posterior,  -- N_DISEASES (update เมื่อเพิ่มโรคใหม่)
  pd.n_matched
FROM per_disease pd
